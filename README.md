...
**Update — v2.x: translation voice speed setting (Na żywo playback only).** The mic-bleed experiment above worked, but the app owner separately reported Gemini's translated voice as noticeably slow, and asked whether it could be sped up — possibly reducing perceived lag as a side effect — without touching VAD, how quickly Gemini starts responding, or the audio sent to Gemini. Explicit constraint: don't implement anything blind — check the current PCM pipeline and whether Android `PlaybackParams`/`AudioTrack` genuinely fits it, with pitch fixed at 1.0, before writing code; use the system mechanism instead of a DSP library if it fits; describe any buffering/latency problem first rather than reaching for a heavier dependency.

Checked Android's own reference docs before writing anything (mirrored at `learn.microsoft.com/en-us/dotnet/api/android.media.playbackparams` and `.audiotrack.playbackparams`, same methodology as every update above):

- `PlaybackParams.setSpeed(float)`/`setPitch(float)` are real, chainable (return `PlaybackParams`), API 23+ (comfortably below this app's minSdk 26). Docs state explicitly: *"Pitch equals 1.0f. Speed change will be done with pitch preserved, often called timestretching."* — exactly the mechanism requested, and confirmation that a naive "play 24kHz PCM at a different declared rate" trick (which WOULD shift pitch) was never the right approach.
- `AudioTrack.setPlaybackParams(PlaybackParams)` is real, callable on a `MODE_STREAM` track, but is documented to fail to apply the parameters if the buffer is too small: *"For speeds greater than 1.0f, the AudioTrack buffer on configuration must be larger than the speed multiplied by the minimum size (`getMinBufferSize()`) to allow proper playback."* This app's `playbackTrack` was never explicitly sized (left to `AudioTrack.Builder()`'s own default) — now fixed, see below.
- `PlaybackParams()` has a plain no-arg constructor, so `applyPlaybackSpeed` always builds a fresh one rather than depending on `getPlaybackParams()`'s undocumented behavior on a not-yet-playing track.

Implementation, entirely inside `LiveTranslationService.kt` and a new small model file — nothing DSP, nothing added as a dependency:

- New `TranslationPlaybackSpeed` enum (`model/TranslationPlaybackSpeed.kt`): `NORMAL` (1.0x), `SLIGHTLY_FASTER` (1.15x), `FASTER` (1.25x, the new default), `QUITE_FAST` (1.35x), `FASTEST` (1.5x) — matching the exact option list requested. New `AppSettingsStore.translationPlaybackSpeed` property (same "read once per session, not retroactive mid-session" convention as `audioCaptureMode`), new Settings card ("Audio & Live" section, right after audio capture mode) with the same Spinner + deferred-listener pattern as every other setting on that screen.
- New `LiveTranslationService.applyPlaybackSpeed(track)`: reads the configured multiplier, builds `PlaybackParams().setSpeed(speed).setPitch(1.0f)`, assigns it to `track.playbackParams`, wrapped defensively (never fatal to an active session over a cosmetic preference). Called right after building EVERY `playbackTrack` this class creates — both `beginCapturePlayback`'s normal path and `rebuildPlaybackTrack` (used by the earpiece-communication routing from the updates above) — so the speed setting applies uniformly regardless of which output route/mode is active, and picks up a Settings change on the next session/route-driven rebuild, same as every other per-session setting here.
- New `LiveTranslationService.playbackBufferSizeBytes()`: `AudioTrack.getMinBufferSize()` × a new `PLAYBACK_BUFFER_SPEED_HEADROOM` (2.5, comfortably above the 1.5x maximum offered speed) constant — now passed via `.setBufferSizeInBytes(...)` to every `AudioTrack.Builder()` call in this class, satisfying `setPlaybackParams()`'s documented requirement. Sized for the maximum offered speed regardless of what's currently configured, so changing the Settings value later never needs a different buffer.

On the latency question specifically: a larger `MODE_STREAM` buffer *capacity* does not by itself add playback latency — only the amount of unplayed audio actually queued in it does, and this class still writes audio in the same chunks it always has as `ServerEvent.AudioChunk` events arrive; nothing about the write pattern changed. What genuinely could add a small amount of latency, and which neither this sandbox nor the docs found give a concrete number for, is the time-stretch algorithm itself needing some internal processing lookahead for quality pitch-preserving output — likely small (real apps use this exact API for the same purpose), but unverified here. Net effect over a sustained sentence should still be less backlog, since output is consumed faster than Gemini produces it — but that is a real-device question, not a documentation one.

Left completely unaffected, per the app owner's explicit constraints: audio sent TO Gemini (`runCaptureLoop`/`micSource` untouched), `CAPTURE_SAMPLE_RATE_HZ`/`PLAYBACK_SAMPLE_RATE_HZ` (the format Gemini is told about and the format tracks are built/written at — PlaybackParams changes speed on TOP of that format, never the format itself), transcription, translation content, VAD, and the EARPIECE/Bluetooth/speaker routing fixed in the updates above.

New string keys (`section_translation_speed`, `translation_speed_description`, `label_translation_speed_1_0`/`_1_15`/`_1_25`/`_1_35`/`_1_5`) added to `values/` and `values-pl/` directly, then translated and injected into all 36 other locales via `scripts/v2_x_translation_speed_translations.py` + `scripts/inject_v2_x_translation_speed_translations.py` (same established pattern) — 266 strings inserted (38 locales × 7 keys), every `values-XX/strings.xml` re-validated with `xmllint --noout` and checked for duplicate/missing keys afterward, alongside `activity_settings.xml`.

Verified in this sandbox: brace/paren balance checker (clean) on every changed/new Kotlin file, `xmllint --noout` on every changed/new XML file (clean), no test files reference the changed private capture/playback functions. Not otherwise testable here — no microphone, speaker, or real `AudioTrack` HAL. Needs on-device confirmation of: (a) the translated voice audibly plays faster at 1.25x (and the other options) without a pitch/chipmunk shift; (b) it still plays correctly, with no glitches or underruns, throughout a live Gemini streaming session, across a route change (headset connect/disconnect) and an `AudioTrack` rebuild; (c) whether it measurably reduces perceived lag, or introduces any of its own, especially at 1.5x; (d) that mic capture, transcription, and the earlier EARPIECE routing fix are all completely unaffected.

**Update — every speed above 1.0x stuttered on real hardware; root-caused against official docs and fixed with an adaptive, backlog-gated speed instead of a fixed one.** The app owner confirmed the speed setting above changed the voice's pace, but reported every speed above 1.0x "stutters as if the buffer is making pauses" (`zacina jak by bufor robił przerwy`), tested with no headphones connected.

Checked `AudioTrack.getUnderrunCount()`'s own reference doc first (mirrored at `learn.microsoft.com/en-us/dotnet/api/android.media.audiotrack.underruncount`, same methodology as every update above) rather than guessing at the cause: *"An underrun occurs if the application does not write audio data quickly enough, causing the buffer to underflow and a potential audio glitch or pop"* — exactly the reported symptom. The root cause: `LiveTranslationService` writes each `ServerEvent.AudioChunk` to `playbackTrack` the moment it arrives, at exactly the pace Gemini delivers it — but the previous update applied a FIXED speed multiplier permanently. Over any sustained stretch of translated speech, Gemini cannot deliver audio faster on average than it plays back in real time, so a track always running at, say, 1.25x drains its own buffer faster than it can ever be refilled and eventually runs dry, repeatedly. That same doc's own suggested fix — "recreate with a larger buffer" — only delays the first underrun; a bigger buffer is still a fixed amount of audio that a permanently-too-fast track will always eventually exhaust, so simply enlarging `PLAYBACK_BUFFER_SPEED_HEADROOM` further would not have actually fixed this.

The real fix: only ever run faster than 1.0x while there is real, already-received backlog to safely speed through — never faster than data has actually arrived, and re-checked continuously instead of applied once and left alone:

- `LiveTranslationService` now tracks `framesWrittenThisTrack` (total frames handed to the current `playbackTrack`) and compares it, after every `AudioChunk` write, against `AudioTrack.getPlaybackHeadPosition()` (documented as frames actually played so far, reinterpreted as unsigned per its own doc). The difference is exactly how much received audio is queued, unplayed, right now — the real backlog, not a guess.
- New `updateAdaptivePlaybackSpeed()`: below `PLAYBACK_BACKLOG_DISENGAGE_SECONDS` (200ms) of backlog, forces 1.0x; at or above `PLAYBACK_BACKLOG_ENGAGE_SECONDS` (500ms), engages the user's configured speed; between the two thresholds, whichever state was already active stays active (hysteresis), so speed doesn't flip back and forth on nearly every chunk near the noisy edge of "roughly empty" — which risked being more audibly glitchy than never speeding up at all.
- `applyPlaybackSpeed(track, speed)` changed from reading the setting itself to taking speed as a plain parameter: `rebuildPlaybackTrack` and `beginCapturePlayback`'s normal path both now always start a freshly built track at 1.0x (zero backlog on a brand new track — starting sped up would underrun immediately), and only `updateAdaptivePlaybackSpeed` ever engages the configured speed, once real backlog justifies it.
- `playbackBufferSizeBytes()` now also sizes for `PLAYBACK_BACKLOG_CAPACITY_SECONDS` (900ms), not just `setPlaybackParams()`'s minimum requirement — the previous buffer was only ever large enough to satisfy that minimum, never large enough to hold a meaningful backlog for the adaptive logic to work with in the first place. A larger buffer *capacity* still does not by itself add latency (unchanged reasoning from the update above) — it only raises the ceiling on how much can ever queue up; what actually queues up is still exactly whatever has arrived from Gemini and not yet played.
- Every speed engage/disengage transition, the measured backlog in milliseconds, and the running `AudioTrack.getUnderrunCount()` value are now logged under a new `TextGateLiveSpeed` logcat tag, so this can be confirmed (or, if still wrong, precisely diagnosed) directly from a real device instead of asserted.

`framesWrittenThisTrack`/`fastSpeedEngaged` are reset every time a new `playbackTrack` is built — both call sites (`beginCapturePlayback`, `rebuildPlaybackTrack`, which the earpiece-communication routing from earlier updates also goes through) — so a route change or mode switch mid-session always starts the new track's backlog tracking cleanly at zero, never carrying over stale state from the previous track. Nothing about the EARPIECE/Bluetooth/speaker routing, mic capture, sample rates, transcription, translation content, or VAD changed — this is scoped entirely to how `translationPlaybackSpeed` gets applied to `playbackTrack`.

Verified in this sandbox with the brace/paren balance checker (clean) on `LiveTranslationService.kt` and `TranslationPlaybackSpeed.kt`. Not otherwise testable here — no real `AudioTrack`/HAL. Needs on-device confirmation, via `adb logcat -s TextGateLiveSpeed`, of: (a) the stutter is gone at every speed option, including 1.5x; (b) fast mode actually engages often enough during a real session to be worth having (if `backlogMs` rarely reaches 500 in the logs, the engage threshold may need lowering); (c) `underruns` stays flat (not increasing) once this is running, confirming the adaptive gate is genuinely never overspeeding; (d) the earlier EARPIECE routing fix and mic-bleed experiment remain completely unaffected.

**Update — the adaptive-speed fix above did NOT resolve the stutter on real hardware; the whole translation voice speed feature is reverted.** The app owner tested the adaptive/backlog-gated version and reported it still stutters at every speed above 1.0x, the same as before, and asked to drop this direction entirely and go back to the version with no playback acceleration.

Rather than attempt a third mechanism without a way to see real backlog/underrun numbers from the device that reported the problem (the `TextGateLiveSpeed` logcat output this update would have needed to diagnose further was never shared), this reverts the feature back out cleanly instead of guessing again:

- `LiveTranslationService.kt`: removed `applyPlaybackSpeed`, `updateAdaptivePlaybackSpeed`, and `playbackBufferSizeBytes` entirely, the `android.media.PlaybackParams` import, the `framesWrittenThisTrack`/`fastSpeedEngaged` tracking fields, the `PLAYBACK_BUFFER_SPEED_HEADROOM`/`PLAYBACK_BACKLOG_*`/`BYTES_PER_PLAYBACK_FRAME` constants, and every `.setBufferSizeInBytes(...)` call on the `AudioTrack.Builder()`s in `beginCapturePlayback` and `rebuildPlaybackTrack` — both now build a plain `MODE_STREAM` track exactly as before this feature existed, with no `PlaybackParams` ever applied and no non-default buffer sizing.
- Deleted `model/TranslationPlaybackSpeed.kt`, the `AppSettingsStore.translationPlaybackSpeed` property (and its `KEY_TRANSLATION_PLAYBACK_SPEED` constant — an already-stored value for a user who tried the setting is simply never read again, harmless orphaned data), and the Settings UI card (`SettingsActivity.setupTranslationPlaybackSpeedSection` + its options list/label mapping, and the matching card in `activity_settings.xml`).
- Removed the 7 `translation_speed_*`/`label_translation_speed_*` string keys from all 40 `values*/strings.xml` files (280 entries total), and deleted the now-unused `scripts/v2_x_translation_speed_translations.py` + `scripts/inject_v2_x_translation_speed_translations.py`.

Everything else from the updates above stays exactly as it was: the EARPIECE/communication routing fix, the STANDARD-mode `VOICE_COMMUNICATION`-without-explicit-AEC mic-bleed experiment, and the headset-disconnect auto-switch-to-speaker behavior are all untouched by this revert — this is scoped entirely to undoing the playback-speed feature and nothing else.

Verified in this sandbox: brace/paren balance checker (clean) on every changed Kotlin file, `xmllint --noout` on every changed XML file (clean), no duplicate string keys in any locale file, and a grep across `app/src/main/` confirms zero remaining references to `TranslationPlaybackSpeed`, `PlaybackParams`, or any `translation_speed_*` string key. Na żywo's translated voice is back to playing at plain 1.0x, unconditionally, with no adjustable speed setting in Settings.

**Update — temporary diagnostic mode added to "Na żywo": Gemini Live turn latency + AudioTrack output backlog measurement.** After reverting the playback-speed feature (the app owner reported it didn't fix the stutter, and asked to drop that direction entirely), the next question is whether Gemini Live's own translation delay is roughly constant or grows during fast/continuous speech — this adds pure measurement, explicitly requested as NOT changing VAD, chunking, routing, playback speed (already removed), or Gemini connection config in any way.

Everything lives in a new, clearly-labeled "TEMPORARY diagnostics" section in `LiveTranslationService.kt` (right above `addStateListener`), gated behind one `private const val LIVE_DIAGNOSTICS_ENABLED = true` flag — flip it to `false`, or delete that section plus every call site it guards, to remove the feature entirely once the measurement answers the question it was added for:

- **Current / average / max latency**: "opóźnienie od ostatniego fragmentu input audio do pierwszego output audio", measured directly rather than approximated — `runCaptureLoop` (the mic capture thread) already sends a chunk to Gemini every time it has one, regardless of session state (Gemini's own VAD decides turns, this app never gates sending), so `diagLastInputSentAtMs` just records the timestamp of the most recent one, updated on every send. `handleServerEvent`'s `ServerEvent.AudioChunk` case already had the exact "a new turn just started" detection this needed (`state == LISTENING` right before transitioning to `TRANSLATING`) — `recordTurnLatency()` hooks that exact existing line, computing `now - diagLastInputSentAtMs` as that turn's latency, with a running sum/count for the average and a running max. Nothing about the existing turn-detection logic was added to or changed, only read from.
- **Audio output backlog**: the request's own formula, `seconds = pendingBytes / (24000 * 2)`. Since `AudioTrack.write()` is blocking and this app has no separate application-level queue in front of it (confirmed while designing this: audio is written directly to `playbackTrack` the instant a `ServerEvent.AudioChunk` arrives, nothing else sits between), the correct way to estimate real buffered audio turned out to be exactly the same mechanism the (now-reverted) speed feature already used to answer this same question: `diagBytesWrittenCurrentTrack` counts bytes actually handed to (and, since `write()` already returned, accepted by) the current track; `AudioTrack.getPlaybackHeadPosition()` (documented as frames actually played so far, reinterpreted as unsigned) gives frames actually consumed. The difference, in bytes, is exactly what's still sitting in `AudioTrack`'s own internal ring buffer — there is no separate queue to estimate, because that ring buffer IS the only queue in this pipeline. This counter resets to 0 every time a new `playbackTrack` is built (both `beginCapturePlayback`'s normal-path branch and `rebuildPlaybackTrack`, shared by the earpiece-routing engage/disengage from earlier updates), so a route/mode switch mid-session never reads as a fake, huge backlog from a track that no longer exists.
- A new `updateDiagnosticsTick()` recomputes and republishes all four numbers every 300ms (within the requested 250-500ms) via `mainHandler.postDelayed`, started at the end of `beginCapturePlayback()` (only while a session actually has audio flowing, never polling while idle between sessions) and stopped — with every counter zeroed, per "Po STOP wyzeruj statystyki" — at the top of `stopCapturePlayback()`.
- A brand new, separate `diagnosticsListeners` list (distinct from the existing `stateListeners`/`notifyListeners()` used for real state changes) carries these updates to `LiveTabController`, specifically so a ~300ms diagnostic tick never forces an unrelated full UI re-render several times a second — `LiveTabController` registers/unregisters a `diagnosticsListener` alongside its existing `stateListener` in the same `onServiceConnected`/`onServiceDisconnected`/`onStop`, and a new `updateDiagnosticsDisplay()` formats the four values with `Locale.US` explicitly (not the device locale — Polish formats decimals with a comma, which would silently break the requested "X.X s" format).
- New "Diagnostics (temporary)" card in `content_live.xml`, appended after the transcript card, with four monospace `TextView`s (`textLiveDiagLatencyCurrent`/`_Average`/`_Max`/`textLiveDiagAudioBacklog`) showing exactly `Latency: X.X s` / `Average: X.X s` / `Max: X.X s` / `Audio backlog: X.X s`. Deliberately hardcoded, unlocalized debug labels set directly in Kotlin — not new string resources — since this card is explicitly temporary and not meant to go through the 40-locale translation pipeline the rest of this app's UI does.

Verified in this sandbox: brace/paren balance checker (clean) on both changed Kotlin files, `xmllint --noout` on the changed layout (clean), and a grep confirming zero leftover references to the reverted `TranslationPlaybackSpeed`/`PlaybackParams` anywhere in the app. Not otherwise testable here — no real `AudioTrack`/HAL/network. Needs on-device confirmation of: (a) the four numbers actually update every ~300ms while a session is running; (b) `Latency`/`Average`/`Max` track sensibly across several turns, especially during fast/continuous speech (the actual question this was built to answer); (c) `Audio backlog` reads near 0 during quiet stretches and rises during any real Gemini burst, without ever going negative or obviously wrong; (d) every number resets to `0.0 s` immediately after STOP; (e) removing headphones / any route switch mid-session doesn't make the backlog reading jump to something absurd.

**Update — the diagnostic mode above didn't give a usable measurement; reverted back to the last known-good state.** The app owner reported "nie da się zmierzyć" (couldn't get a measurement out of it) and asked to go back to the best-working version — i.e. before this diagnostics card existed, on top of everything already reverted in the update before it (no playback-speed feature).

Clean revert, same approach as the playback-speed rollback above:

- `LiveTranslationService.kt`: removed the entire "TEMPORARY diagnostics" section (above `addStateListener`) — `diagLastInputSentAtMs`, `diagBytesWrittenCurrentTrack`, all `diagLatency*`/`diagAudioBacklogSeconds` fields, `addDiagnosticsListener`/`removeDiagnosticsListener`/`notifyDiagnosticsListeners`, `diagTickRunnable`, `recordTurnLatency`, `updateDiagnosticsTick`, `resetDiagnostics` — every call site it added inside `handleServerEvent`'s `AudioChunk` case, `runCaptureLoop`, `beginCapturePlayback` (both branches), `rebuildPlaybackTrack`, and `stopCapturePlayback`, the `LIVE_DIAGNOSTICS_ENABLED`/`DIAG_TICK_INTERVAL_MS`/`DIAG_BYTES_PER_FRAME` constants, and the now-unused `java.util.concurrent.atomic.AtomicLong` import.
- `LiveTabController.kt`: removed `diagnosticsListener`, its registration/unregistration in `onServiceConnected`/`onServiceDisconnected`/`onStop`, and `updateDiagnosticsDisplay()`.
- `content_live.xml`: removed the "Diagnostics (temporary)" card and the `xmlns:tools` namespace declaration that only existed for its `tools:text` preview attributes.

Every other update above this one (the EARPIECE/communication routing fix, the STANDARD-mode mic-bleed experiment, headset-disconnect auto-switch-to-speaker, and the playback-speed revert) is untouched — this undoes only the diagnostics card and its measurement plumbing, nothing else.

Verified in this sandbox: brace/paren balance checker (clean) on both changed Kotlin files, `xmllint --noout` on the changed layout (clean), and a case-insensitive grep confirming zero remaining `diag`-prefixed identifiers anywhere in `LiveTranslationService.kt`/`LiveTabController.kt`/`content_live.xml` (the pre-existing, unrelated `logRoutedDevice` earpiece-routing diagnostic from an earlier update is untouched and still present, as expected). Na żywo is back to exactly the state before any latency/backlog measurement work started.

**Update — app now defaults to English out of the box, picker language names now show their `?code` typed-trigger suffix, the two audio-capture-mode labels are renamed, and Rozmowa's language-swap button now swaps the actual language values like Tłumacz's does.** Four separate requests from the app owner, all UI/i18n-scoped, none touching the Live/Rozmowa audio pipeline, VAD, or Gemini config.

- **Default language is now English, for the whole app.** `Languages.DEFAULT` changed from `"pl"` to `"en"` — this is the project's single source of truth for "what language does the app show/translate into when nothing has been explicitly chosen," so this one change is enough by itself: `LocaleHelper.applyOverride()`/`resolvePreferredLanguage()` already read `AppSettingsStore.appInterfaceLanguage`, falling back to `Languages.DEFAULT` whenever it's unset. What did need to change: both functions previously fell back further, through a `matchToSupportedLanguage(deviceLocale)` best-effort matcher, to whatever language the device's own system locale happened to resolve to — that matcher is now deleted entirely, and a `null` stored preference resolves straight to `Languages.DEFAULT` (English). A fresh install, or any device set to a non-English system locale, now shows the app in English until the user picks something else in Settings' "App interface language" picker — manual selection there is completely unaffected, this only changes the unset-preference fallback.
- **Picker labels now show their typed-trigger shortcut.** New `pickerLabel()` extension function on `SupportedLanguage` in `Languages.kt` — `"$nativeName ?$code"`, e.g. `"English ?en"`, `"Polski ?pl"` — applied to every language-picker adapter's label list in the app: Settings' own language spinner, Tłumacz's source and target spinners, and Rozmowa's two spinners (its "Automatycznie" auto-detect entry in Tłumacz's source list is unaffected, since it isn't a `SupportedLanguage`). Deliberately a separate function from plain `nativeName`, not a change to `nativeName` itself, since status/display-only text elsewhere (Na żywo's "wykryto: ..." detected-language line, Rozmowa's "A → B" direction label) should keep showing just the plain name, not a picker-only suffix.
- **Audio-capture-mode labels renamed.** In Settings' "Audio capture mode" section: "Echo-cancelled (recommended)" → "Multimedia speaker" (EN) / "Redukcja echa (zalecane)" → "Głośnik multimedialny" (PL); "Standard (lower latency)" → "Call speaker" (EN) / "Standardowy (mniejsze opóźnienie)" → "Głośnik rozmowy" (PL) — new one-off script `scripts/rename_audio_capture_labels.py` replaced the two existing string keys' (`label_audio_capture_echo_cancelled`/`label_audio_capture_standard`) VALUES in place across all 40 `values*/strings.xml` files, translated per locale (e.g. `de`: "Multimedia-Lautsprecher"/"Gesprächslautsprecher"; `ja`: "メディアスピーカー"/"通話スピーカー"; `zh`/`zh-rCN`: "媒体扬声器"/"通话扬声器"). Unlike the project's existing `vX_Y_..._translations.py` + `inject_..._translations.py` scripts (which only ever insert brand-new keys and fail loudly if one is unexpectedly missing), this script replaces an EXISTING key's inner text via regex and fails loudly if either key is unexpectedly absent from a locale file — nothing else about `AudioCaptureMode` (its enum values, `fromPrefValue`/`prefValue`, or anything reading `AppSettingsStore.audioCaptureMode`) changed; this is a display-text-only rename.
- **Rozmowa's language swap now swaps the actual language values, matching Tłumacz.** Previously, `ConversationTabController` held two fixed language slots (`languageA`, `languageB`) plus a `directionAtoB: Boolean` flag; "swap" just flipped that flag, changing which slot counted as source vs. target while the two spinners themselves never moved. That's now gone: `directionAtoB` is deleted, `languageA`/`languageB` are the two spinner slots exactly as before, and the swap button now calls a new `swapLanguages()` that exchanges `languageA`'s and `languageB`'s values directly and moves both spinners' selections to match — the same pattern `TranslateTabController.swapLanguages()` already used for Tłumacz's source/target spinners. `currentTargetLanguage()` simplifies to always `languageB` (there's no more "which slot is currently read as target" branch — swapping changes what's IN the slot instead), and `updateDirectionLabel()` simplifies the same way, always showing `languageA → languageB`. The shared spinner-selection-changed listener was extracted into `attachLanguageSpinnerListeners()` so `setupLanguageSpinners()` (initial attach) and `swapLanguages()` (re-attach after swapping) install identical behavior without duplicating it; `swapLanguages()` detaches both spinners' listeners before calling `setSelection()` on either and re-attaches them after, specifically because `Spinner.setSelection()` fires the exact same `OnItemSelectedListener` a user tap would — leaving the listeners attached during a swap would have restarted the live Gemini session twice (once per spinner) for a single button press instead of once.

Verified in this sandbox: brace/paren balance checker (clean) on every changed Kotlin file (`Languages.kt`, `LocaleHelper.kt`, `AppSettingsStore.kt` — doc-comment-only change, `SettingsActivity.kt`, `TranslateTabController.kt`, `ConversationTabController.kt`), `xmllint --noout` on all 40 `values*/strings.xml` files (clean), and a grep confirming every language-picker adapter site in the app now uses `.pickerLabel()` while every status/display-only language text still uses plain `.nativeName`. Not otherwise testable here — no compiler/SDK in this sandbox. Needs on-device confirmation of: (a) a fresh install (or a device set to a non-English system locale) shows the app in English before any manual language choice is made; (b) every language picker (Settings, Tłumacz's two spinners, Rozmowa's two spinners) shows the `?code` suffix; (c) the audio-capture-mode section shows the new "Multimedia speaker"/"Call speaker" labels; (d) Rozmowa's swap button visibly exchanges which language sits in the top vs. bottom spinner (not just the direction arrow text) and, if a Live session is active, reconnects with the new target language.

**Update — fixed a CI build failure introduced by the audio-capture-mode rename above.** GitHub Actions' `assembleDebug` failed at `:app:mergeDebugResources` — aapt's own error, `Failed to flatten XML for resource 'label_audio_capture_standard' with error: Invalid unicode escape sequence in string`, pointed at `values-fr/strings.xml`. Cause: the new French text, "Haut-parleur d'appel", has a plain, unescaped apostrophe — Android string resources require `\'` (every other apostrophe already in this file, e.g. `l\'appui`, `d\'autre`, was already escaped this way; the new translation just didn't follow that same rule). `xmllint --noout`, run as part of verifying the original change, didn't catch this because a bare `'` is perfectly valid XML — this is an aapt-specific resource-compiler rule, not an XML-well-formedness one.

Fixed the one line (`Haut-parleur d'appel` → `Haut-parleur d\'appel`) and the same string in `scripts/rename_audio_capture_labels.py`'s `TRANSLATIONS` dict, so a future re-run reproduces the fix rather than the bug. Also wrote a one-off verification pass across all 40 `values*/strings.xml` files scanning every `<string>` value for an apostrophe not preceded by a backslash — the French line was the only hit; nothing else in the app's 40 locale files has this problem. No other change from the update above this one was affected — this is a one-character fix to a single locale file plus the script that generated it.

**Update — two follow-up corrections from the app owner on the update above: the `?code` picker suffix should be Settings-only, and the audio-capture-mode description text still named the OLD option labels.**

- **`?code` suffix scoped back to Settings only.** The previous update applied `pickerLabel()` to every language spinner in the app (Settings, Tłumacz's two spinners, Rozmowa's two spinners); the app owner clarified it should show only in Ustawienia (Settings), not in every other tab. Reverted `TranslateTabController.setupLanguageSpinners()` and `ConversationTabController.setupLanguageSpinners()` back to plain `Languages.ALL.map { it.nativeName }` / `languages.map { it.nativeName }` (and dropped their now-unused `import com.textgate.ai.model.pickerLabel`) — `SettingsActivity.setupLanguageSection()` is untouched and remains the only call site. `pickerLabel()` itself (in `Languages.kt`) is unchanged; only its doc comment was rewritten to say explicitly that it's Settings-only, not a general picker helper, so this doesn't get re-applied elsewhere by mistake later.
- **Audio-capture-mode description text updated to match the renamed options.** `label_audio_capture_echo_cancelled`/`label_audio_capture_standard`'s VALUES were renamed two updates ago ("Echo-cancelled (recommended)" → "Multimedia speaker", etc.), but `audio_capture_mode_description` — the explanatory sentence right below the two options in Settings — was never touched, so it kept naming the options by their old labels in every one of the 40 locales (e.g. English still read `"Echo-cancelled" stops the microphone...`/`"Standard" is faster...` even though the picker above it now says "Multimedia speaker"/"Call speaker"). New one-off script `scripts/fix_audio_capture_mode_description.py` fixes this properly rather than by hand: each locale's description names things inside that locale's own quote convention (straight `\"..\"` in most, «...»/»...« reversed (Slovenian)/„...“ or „...”/「...」 in others), and several locales (`ar`, `bg`, `ja`, `lt`, `lv`, `ru`, `th`, `uk`, `zh`, `zh-rCN`) additionally quote the two TAB names ("Rozmowa"/"Na żywo", translated) earlier in the same sentence using the identical quote style — so a description can contain either exactly 2 or exactly 4 quoted spans, and the script auto-detects which quote style is in play and always takes the LAST two matches (confirmed against every one of the 40 files before running) as the two mode names to replace, leaving any tab-name quoting and the rest of the sentence untouched. French additionally needed its own quoted phrases to keep the literal leading/trailing space its typography convention uses inside `« ... »` (confirmed against every other `« ... »` occurrence already in that file) — the script now preserves whatever leading/trailing whitespace the OLD quoted text had around the new text automatically, rather than a one-off hardcoded fix, so this generalizes if another locale needs it later.

Verified in this sandbox: brace/paren/bracket balance checker (clean) on `TranslateTabController.kt`, `ConversationTabController.kt`, and `Languages.kt`; `xmllint --noout` on all 40 `values*/strings.xml` files (clean); a full-file apostrophe scan across all 40 locale files confirming zero unescaped `'` remain anywhere (not just the one French line fixed above); and a grep confirming no locale file still names either audio-capture-mode option by its pre-rename text. Needs on-device confirmation of: (a) the `?code` suffix now shows only in Settings' language picker, with Tłumacz's and Rozmowa's spinners back to plain language names; (b) the audio-capture-mode description in Settings now correctly names "Multimedia speaker"/"Call speaker" (or the matching translation) instead of the old option names, in whatever language the device is set to.
