# -*- coding: utf-8 -*-
"""
Injects the v2.0.1 audio-capture-mode string translations
(v2_0_1_audio_capture_translations.py) into every values-XX/strings.xml
locale file that is missing them — same purpose and escaping rules as
inject_v2_translations.py, scoped to just the 4 new keys this update added
(section_audio_capture_mode, audio_capture_mode_description,
label_audio_capture_echo_cancelled, label_audio_capture_standard), so CI's
...
Run from the repo root: python3 scripts/inject_v2_0_1_audio_capture_translations.py
"""
import re
import sys
...
from v2_0_1_audio_capture_translations import TRANSLATIONS  # noqa: E402

REPO_ROOT = Path(__file__).parent.parent
RES_DIR = REPO_ROOT / "app" / "src" / "main" / "res"

NEW_KEYS = [
    "section_audio_capture_mode",
    "audio_capture_mode_description",
    "label_audio_capture_echo_cancelled",
    "label_audio_capture_standard",
]

# locale folder suffix -> which TRANSLATIONS entry to use (identical mapping
# to inject_v2_translations.py, including the pt-rBR/zh-rCN sharing convention)
...
def main():
    ...
    for locale_dir, translation_key in LOCALE_TO_TRANSLATION_KEY.items():
        ...
        lines = ["    <!-- v2.0.1: audio capture mode setting -->"]
        not_found = []
        ...
        if not_found:
            raise SystemExit(
                ...
                "in v2_0_1_audio_capture_translations.py - aborting so nothing is silently skipped."
            )
